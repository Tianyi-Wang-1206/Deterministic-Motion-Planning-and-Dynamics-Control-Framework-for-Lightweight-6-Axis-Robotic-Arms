...
from lite6_interfaces.action import IndustrialMotion
...
class ROS2Worker(QThread):
    ...
    def __init__(self):
        ...
        self.move_client = ActionClient(self.node, IndustrialMotion, 'industrial_motion')
    ...
    def _array_to_pose_msg(self, pose_arr):
        """Helper: convert [X, Y, Z, R, P, Y] to geometry_msgs/Pose"""
        p = Pose()
        p.position.x = float(pose_arr[0])
        p.position.y = float(pose_arr[1])
        p.position.z = float(pose_arr[2])
        rpy_vec = np.array([float(pose_arr[3]), float(pose_arr[4]), float(pose_arr[5])])
    ...
    def send_industrial_goal(self, goal):
        with self.state_lock:
    ...
    def execute_joint_target(self, target_q, v_scale, a_scale, callback=None):
        ...
        goal.command_type = IndustrialMotion.Goal.MOVEJ
        goal.joint_target = [float(q) for q in target_q]
    ...
    def execute_circular_target(self, target_pose, aux_pose, v_scale, a_scale):
        goal = IndustrialMotion.Goal()
        goal.command_type = IndustrialMotion.Goal.MOVEC
        goal.pose_target = self._array_to_pose_msg(target_pose)
        goal.pose_aux = self._array_to_pose_msg(aux_pose)
        goal.velocity_scale = float(v_scale)
        goal.acceleration_scale = float(a_scale)
        self.send_industrial_goal(goal)
    ...
    def goal_cb(self, future):
        ...
        with self.state_lock:
            if not gh.accepted:
                self.set_state(Lite6StateMachine.SOFT_ERROR, "Plan Rejected by Industrial Planner!")
                return
    ...
    def result_cb(self, future):
        ...
        with self.state_lock:
            if res.error_code == 1: # SUCCESS
                if self.state in [Lite6StateMachine.EXECUTING, Lite6StateMachine.RECOVERING]:
                    self.set_state(Lite6StateMachine.IDLE)
            elif res.error_code == -7:  # PREEMPTED / ABORTED
                pass 
            else:
                if self.state != Lite6StateMachine.ERROR: 
                    self.set_state(Lite6StateMachine.SOFT_ERROR, f"Planner Fault: {self.get_planner_error_str(res.error_code)}")
    ...
    def sysid_result_cb(self, future, next_step_callback):
        ...
        with self.state_lock:
            if res.error_code == 1:
                ...
                self.set_state(Lite6StateMachine.SOFT_ERROR, f"SysID alignment movement failed: {res.error_code}")
...
class PyQtHMI(QMainWindow):
    ...
    def gui_reset_fault(self):
        ...
        # Build recovery goal using the new Industrial protocol
        goal = IndustrialMotion.Goal()
        goal.command_type = IndustrialMotion.Goal.MOVEJ
        goal.joint_target = [float(q) for q in q_curr_copy]
    ...
    def gui_unlock_cb(self, future):
        gh = future.result()
        if not gh.accepted:
            self.worker.set_state(Lite6StateMachine.ERROR, "Recovery motion rejected by Planner.")
            return
    ...
    def gui_finalize_recovery(self, future):
        ...
        if res.error_code == 1: # SUCCESS
            ...
            self.worker.set_state(Lite6StateMachine.ERROR, f"Recovery motion failed: {res.error_code}")
